...
void HGCalECONTConfig::serialize(Archive& ar, const unsigned int) {
  ar & BOOST_SERIALIZATION_NVP(density);
  ar & BOOST_SERIALIZATION_NVP(dropLSB);
  ar & BOOST_SERIALIZATION_NVP(select);
  ar & BOOST_SERIALIZATION_NVP(stcType);
  ar & BOOST_SERIALIZATION_NVP(eportTxNumen);
  ar & BOOST_SERIALIZATION_NVP(sumType);
  ar & BOOST_SERIALIZATION_NVP(calv);
  ar & BOOST_SERIALIZATION_NVP(tcMux);
  ar & BOOST_SERIALIZATION_NVP(offset);
}

template <class Archive>
void HGCalTDAQConfig::serialize(Archive& ar, const unsigned int) {
  ar & BOOST_SERIALIZATION_NVP(tdaqBlockHeaderMarker);
  ar & BOOST_SERIALIZATION_NVP(econts);
}

template <class Archive>
void HGCalTriggerFedConfig::serialize(Archive& ar, const unsigned int) {
  ar & BOOST_SERIALIZATION_NVP(tdaqs);
  ar & BOOST_SERIALIZATION_NVP(econtSwapOffset);
  ar & BOOST_SERIALIZATION_NVP(elinksMap);
}

template <class Archive>
void HGCalTriggerConfiguration::serialize(Archive& ar, const unsigned int) {
  ar & BOOST_SERIALIZATION_NVP(feds);
}
